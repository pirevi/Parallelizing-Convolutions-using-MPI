...
int main(int argc, char** argv) {   
	...
	/* Neighbours */
	// each of the processes have neighbouring processes as the image is divided 
	// into sub-sections and each subsection gets alloted to single process.
	// each proceess will have these values of north, south, west and east, that signify 
	// about its neighbouring processes.
	int north = -1;
	int south = -1;
	int west = -1;
	int east = -1;
    ...
    /* Checking arguments */
	// checking the input arguments given by the user in the console 
	// and assigning them space and variable values.
    if (process_id == 0) {
	...
	/* Init filters */
	// initializing the gaussian blur filter
	...
	// src and dst will store image as arrays
	...
	/* Parallel read */
	// reading the part of the image by an indivisual prrocess
	...
	/* Compute neighbours */
	// north south east and west signify about the neighbouring processes to the current process
    if (start_row != 0)
	...
	/* MPI barrier for all processes to finish upto this point*/
	MPI_Barrier(MPI_COMM_WORLD);

	/* Get time before */
	// Timer for iteration loops where in each iteration the filter convolves over the entier image once.
	...
	for (t = 0 ; t < loops ; t++) {
		...
		// The inner data is the one that does not include the boundaries. This undergoes convolution here 
		...
		/* swap arrays */
	// swapping arrays means updating the new dst array with src array, after the iteration blurs the image.
		tmp = src;
        src = dst;
        dst = tmp;
	}
	...
	/* Parallel write */
	// combining all parallel sections
	...
	/* Get times from other processes and print maximum time elapsed*/
    if (process_id != 0)
...
// the convolute function is called during the convolution of part of the image by a process, It calls another
// convolute_grey or convolute_rgb function based on the imagetype, grey or rgb.
...
// usage function is mainly made to check and assign variable values the input arguments provided on the console.
...
// the following algorithm will provide a best value parameter, which is further used to
// division of the images between processes in the best way possible.
